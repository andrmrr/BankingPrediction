# BankingPrediction
This project conducts an analysis over a **Bank Marketing dataset**, which compiles call information from a Portuguese banking institution. The main objective of the analysis is predicting **whether a client will subscribe to a term deposit**. This classification task relies on various categorical features, such as the client’s education level, occupation, and marital status. Additionally, numerical features like the client’s age and average account balance are considered in the analysis. This was a semester-long project for the course Multivariate Analysis conducted in a group of five students using **R programming language**. The final report is included along with the code, data and additional files. A summary of the project is shown here:
...
6. Hierarchical clustering gave us the following 5 clusters, which were subsequently profiled:
  Cluster 1: Married professionals
  Cluster 2: Clients with distinctive financial behaviour
  Cluster 3: Age-specific demographic group
  Cluster 4: Higher-education clients
  Cluster 5: Clients characterized by loan-related behaviours
